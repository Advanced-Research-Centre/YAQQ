...
def cost_func(points, agent1_gateset, thetas, max_depth, recursion_degree):
    ...
    pf01_db, dep01_db, pf02_db, dep02_db = fidelity_per_point(points, skd1, skd2)
    # ===> Evaluate GS2 based on cost function
    cfn = cfn_calc(pf01_db, dep01_db, pf02_db, dep02_db)
    return cfn

def fidelity_per_point(points, skd1, skd2):
...
if __name__ == "__main__":
    ...
    # Uh = UGate("Uh", h_U_mat)
    # Ut = UGate("Ut", t_U_mat)
    # angle = np.load('opt_ang.npy')
    # U3 = UGate("U3", qiskit_U3(angle[0], angle[1], angle[2]))

    # agent1_gateset = [Uh, Ut, U3]
    # gbs = gen_basis_seq()
    # agent1_gateseq = gbs.generate_basic_approximations(agent1_gateset, 3) 
    # skd1 = SolovayKitaev(recursion_degree=2,basic_approximations=agent1_gateseq)
